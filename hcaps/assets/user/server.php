...
if(count($errors) == 0)
{	
	// $per_password= password_hash($password_1, PASSWORD_DEFAULT);
	$insert = "INSERT INTO requests_tbl (name, lname, mname, suffix, bday, age, houseNo, street, brgyid, reqrole_id, username, password, email, date_requested) VALUES (:name, :lname, :mname, :suffix, :bday, :age, :houseNo, :street, :brgyid, :reqrole_id, :username, :password, :email, :date_requested)";
...
echo '
<script>
window.location.href=\'register_response.php\';
</script>';
...
if(empty($per_username)){
	array_push($errors, "Username is required");
}

if(empty($per_password)){
	array_push($errors, "Password is required");
}

if(count($errors) == 0)
{	
	$select = "SELECT * FROM user_tbl WHERE username='$per_username'";
	...
	if($role!='head_admin' || $role!='patient')
	{
		// if(password_verify($per_password, $hashpassword)){
		if($per_password == $user_password){
		$_SESSION['username'] = $per_username;
		$_SESSION['success'] = "You have signed in successfully!";
		header('location: ../user/poblacion_adm.php');
		}
	}
